...
class OrderController {
	 ...
	 */
	OrderController(OrderManagement<Order> orderManagement, CustomerManagement customerManagement) {
	...
	String buy(@ModelAttribute Cart cart, @LoggedIn Optional<UserAccount> userAccount, RedirectAttributes redirectAttributes) {

		return userAccount.map(account -> {

			Customer customer = customerManagement.findByUserAccount(account)
					.orElseThrow();
			int age = customer.getAge();

			int highestFsk = 0;
			for (CartItem item : cart) {
				if (item.getProduct() instanceof Disc disc) {
					highestFsk = Math.max(highestFsk, disc.getFsk());
				}
			}

			boolean ageMeetsFsk = age >= highestFsk;

			if (!ageMeetsFsk) {
				redirectAttributes.addFlashAttribute("ageError", true);
